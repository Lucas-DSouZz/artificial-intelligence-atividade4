...
void initParticle(particle* p, int dim, double* x_0, int tmax){
/*
...
*/
    ...
    double* aux_v = p->v;
    //adicional ///velocidade anterior* peso de inércia
    double w = WMAX - ((WMAX - WMIN) / p->tmax) * p->t;
    double* aux_VxW = malloc(p->dim * sizeof(double));
    copyArray(aux_VxW, aux_v, p->dim);
    for(int i = 0; i < p->dim ; i++){ // multiplicando o array V da velocidade anterior pelo escalar w
        aux_VxW[i] = aux_VxW[i]*w;
    }


    //Atualiza velocidade
    double* aux_sum = arraySum(aux_mul1,aux_mul2,p->dim);
    p->v = arraySum(aux_VxW,aux_sum,p->dim);
    //Libera memória
    free(aux_sub1);
    free(aux_sub2);
    free(aux_mul1);
