...
*/
    ...
    double* old_v = p->v;
    double* new_v = arraySum(old_v, aux_sum, p->dim);
    
    p->v = new_v;

    //  Inicio mudança
    //  Norma é a norma da velocidade da partícula
    int norma = norm(p->v, p->dim);
    if (norma > VMAX) {
        double* limited = arrayDiv(p->v, (norma * VMAX), p->dim);
        free(p->v);
        p->v = limited;
    }
    //  Fim mudança
